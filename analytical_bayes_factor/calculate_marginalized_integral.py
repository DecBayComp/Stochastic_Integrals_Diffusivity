...
def calculate_marginalized_integral(zeta_t, zeta_sp, n, n_pi, v, E):
	"""
	...
	for zeta_ind in range(zeta_length):
		...
		if np.isclose(zeta_sp_cur, 0, rtol = tol, atol = tol):
			result = (v + E * zeta_t_cur ** 2.0) ** (-(n + n_pi - 3) / 2.0)
		...
		def integrate_me(l):
			return (v + E * (l * zeta_sp_cur - zeta_t_cur) ** 2.0) ** (-(n + n_pi - 3) / 2.0)
