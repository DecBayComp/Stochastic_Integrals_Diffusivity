...
def plot_zeta_t_perp(n_pi, us):
	...
	for u_ind in range(us_count):
		...
		downstairs = np.multiply(np.power(B_threshold / etas, 1/ps), etas ** 2) - 1
	...
	for u_ind in range(us_count):
		...
		ax.plot(ns, zeta_t_perp[u_ind, :], linestyle = style_sequence[u_ind], label = str_label)
