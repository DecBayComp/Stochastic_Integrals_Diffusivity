...
function out = D1_D2_simple_Hanggi_integrand_function(l_ind, bin, D1, D2, fD, t_step, x_step, kBT)
...
%% Calculate
% 1. Bin j-1
log_out = log_out + bin_sigma_squared_log_posterior_func (l_ind, bin - 1, 2 * D1 * t_step);
% 2. Bin j
log_out = log_out + bin_mu_sigma_log_pdf_func (l_ind, bin, ...
    fD * t_step / kBT + (D2 - D1) * t_step / x_step, ...
    2 * D2 * t_step);
% 3. D-independent factor
log_out = log_out + log(4) + 3 * log(t_step) - log(kBT);
