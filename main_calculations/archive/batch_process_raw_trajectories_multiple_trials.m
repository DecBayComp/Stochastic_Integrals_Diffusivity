...
D_PRECISION = 1e-5;
D_ABS_MAX = 1;
bl_find_marginalized_fD_error_bars = true;
...
MAP_bck_fD_Hanggi_mean = cell(1, lambda_count);
s1 = 4;
for l_ind = 1:lambda_count
    ...
    MAP_bck_fD_Hanggi_mean{l_ind} = zeros(s1, x_bins_number_saved(l_ind));
end;
...
for trial = 1:trials
    ...
    % Calculate in bins
    for l_ind = 1:lambda_count
        data_struct.D_theor_data{l_ind} = D_func(selected_D_case, data_struct.x_bins_centers_saved{l_ind}, L);
    ...
    for l_ind = 1:lambda_count
        ...
        for bin = 1:x_bins_number_saved(l_ind)
            ...
            D_inference = find_confidence_interval(log_function_to_minimze, [D_PRECISION, D_ABS_MAX], true, MLE_guess, data_struct.D_theor_data{l_ind}(bin));
            % Save
        ...
        parfor bin = 1:x_bins_number_saved(l_ind)
            ...
            % Find confidence intervals
            fD_divine_inference = find_confidence_interval(log_function_to_minimze, [- fD_ABS_MAX, fD_ABS_MAX], true, MLE_guess, data_struct.fD_theor_data{l_ind}(bin));
            % Save
            ...
            % Find confidence intervals
            fD_Ito_inference = find_confidence_interval(function_to_minimze, [- fD_ABS_MAX, fD_ABS_MAX], true, MLE_guess, data_struct.fD_theor_data{l_ind}(bin));
    %         fprintf('FINISHED: Calculating ITO inference\n');
            ...
            % Find confidence intervals
            fD_Stratonovich_inference = find_confidence_interval(function_to_minimze, [- fD_ABS_MAX, fD_ABS_MAX], true, MLE_guess, data_struct.fD_theor_data{l_ind}(bin));
            % Save
            ...
            fD_MLE_marginalized = find_confidence_interval(function_to_minimze, [- fD_ABS_MAX, fD_ABS_MAX], bl_find_marginalized_fD_error_bars,...
                MLE_guess, data_struct.fD_theor_data{l_ind}(bin));
            % Save
...
%% Calculate the fail rate of each prediction given the known error intervals
for l_ind = 1:lambda_count
    ...
    for trial = 1:trials
        ...
        outside_count_marginalized = outside_count_marginalized + double(theory < estimate(1, :) - estimate(2, :)...
            | theory > estimate(1, :) + estimate(3, :));
%         % Direct Hanggi
%         estimate = trials_data{trial}.MAP_bck_fD_Hanggi{l_ind};
%         outside_count_Hanggi = outside_count_Hanggi + double(theory < estimate(1, :) - estimate(2, :)...
%             | theory > estimate(1, :) + estimate(3, :));
    end;
...
%     data_struct.UR_bck_Hanggi{l_ind} = outside_count_Hanggi / trials;
end;
