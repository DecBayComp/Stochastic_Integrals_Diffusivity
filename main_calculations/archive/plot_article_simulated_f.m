


%% Globals
global L;
global max_f_case_number;

%% Constants
load_constants;
x_over_L_min = -1/2;
x_over_L_max = 1/2;
x_over_L_step = 0.1;

% golden_ratio = (1+sqrt(5))/2;
height_to_width_ratio = 1/1.5;  % 1/1.3


%% Initialize figure
h_fig = figure(9);
set_article_figure_size(h_fig, height_to_width_ratio, 3);
clf;
hold on;
default_color_order = get(gca,'colororder');
markers_list = {'-o','-s','-d','-^','-v'};
legend_position = [0.38,    0.54,    0.14,    0.30];




%% Initialize x mesh
x_mesh = (x_over_L_min:x_over_L_step:x_over_L_max) * L;
x_mesh_length = length(x_mesh);

%% Calculate force for each case
f_data = zeros(max_f_case_number, x_mesh_length);
for f_case = 1:max_f_case_number
    f_data(f_case, :) = f_func(f_case, x_mesh, L);
end;


%% Plot
for f_case = 1:max_f_case_number
    plot(x_mesh/L, f_data(f_case, :), markers_list{f_case}, 'LineWidth', 2,...
    'color', default_color_order(f_case, :), ...
    'MarkerSize', marker_size, 'MarkerFaceColor', default_color_order(f_case, :));
end;


%% Label
y_lim_vec = [-10, 10];
ylim(y_lim_vec);
box on;
grid on;
xlabel('x');
ylabel('f(x)');


%% Add a vertical line at a selected x location
x = selected_x_over_L * ones(1, 2);
y = y_lim_vec;
plot(x, y, '--k', 'LineWidth', 2);


%% Legend
str_legend = {'F1', 'F2', 'F3', 'F4', 'F5'};
h_legend = legend(str_legend, 'FontSize', font_size - 5, 'location', 'northeastoutside'); %, 'Position', legend_position);


%% Save figure
% Prepare printer
h_fig.PaperPositionMode = 'auto';
h_fig.Units = 'Inches';
fig_pos = h_fig.Position;
set(h_fig, 'PaperUnits','Inches','PaperSize',[fig_pos(3), fig_pos(4)]);

% Set filename
output_filename = 'Simulated_forces.pdf';
output_full_path = strcat(output_figures_folder, output_filename);
print(h_fig, output_full_path, '-dpdf', '-r0');







