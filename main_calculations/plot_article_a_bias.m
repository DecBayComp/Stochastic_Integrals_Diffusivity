...
function plot_article_a_bias(data_struct, fig_count, bl_save_figures)
...
for lambda_type = 1:lambda_types_count
    ...
    plot(data_struct.MAP_bb_prime_regular_interp(indices),...
        (data_struct.MAP_a_mean(lambda_type, indices, convention, 1) - data_struct.a_theor_data(indices)'), markers_list{convention},...
    ...
    if lambda_type == 1
        ...
        ylabel('$\langle \delta a \rangle$, $\mu \mathrm{m/s}$ ', 'interpreter', 'latex');
    end;
    ...
    % Labels
    xlabel('$\langle bb'' \rangle$, $\mu \mathrm{m/s}$', 'interpreter', 'latex');
