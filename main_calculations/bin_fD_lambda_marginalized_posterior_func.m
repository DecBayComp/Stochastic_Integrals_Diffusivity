...
function result = bin_fD_lambda_marginalized_posterior_func(data_struct, bin, fD, D_grad, str_direction)
...
for ind = 1:input_length
    ...
    tmp_result = 0;
    integrand_function_wrap = @(lambda) bin_mu_posterior_func(data_struct, bin, fD(ind) * t_step / kBT + lambda * t_step * D_grad, str_direction);
    for k1 = 1:count-1
