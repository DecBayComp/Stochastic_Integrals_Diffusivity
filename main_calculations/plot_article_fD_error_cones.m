...
%     indices = data_struct.x_bins_centers_saved{l_ind} >= -100;
    % Divine
    ...
    plot(data_struct.x_bins_centers_saved{l_ind}(indices), difference_rel, markers_list{1}, 'markers', marker_size);
    % Ito
    ...
    difference_rel = abs(difference ./ half_error);
    plot(data_struct.x_bins_centers_saved{l_ind}(indices), difference_rel, markers_list{2}, 'markers', marker_size);
    % Stratonovich
    ...
    % Identity
    x_lim_vec = xlim();
    plot(x_lim_vec, x_lim_vec * 0 + 1, '--k');
    % Add fill
    x = [x_lim_vec, flip(x_lim_vec)];
    y = [-0.5, -0.5, 1, 1];
    h_r = patch(x, y, fill_color);
    h_r.EdgeColor = 'none';
    alpha(h_r, fill_opacity);
...
%     xlim(x_lim_vec);
    ylim(y_lim_vec);
    ylabel('$2\delta(fD) / w$', 'interpreter', 'latex');
    xlabel('x');
